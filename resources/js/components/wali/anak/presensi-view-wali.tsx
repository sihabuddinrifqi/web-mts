...
export default function MonitorPresensiWali({ siswaId, isOpen, onOpenChange }: MonitorPresensiWaliProps) {
  ...
  useEffect(() => {
    if (isOpen && siswaId && !siswaData && !isLoading) {
      ...
      fetch(route('api.presensi.siswa', { id: siswaId }))
        .then((response) => {
          if (!response.ok) {
            throw new Error(`Gagal mengambil data presensi. Status: ${response.status}`);
          }
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {Object.keys(groupedPresensi).length > 0 ? (
                  Object.keys(groupedPresensi).map((pelajaran) => {
                    ...
                    return (
                      <tr key={pelajaran} className="border-t">
        ...
        <DialogFooter>
          <div className="flex w-full items-center justify-end">
            <Button asChild disabled={!siswaData?.nis}>
              <a
                href={siswaData?.nis ? route('presensi.report', siswaData.nis) : '#'}
                target="_blank"
